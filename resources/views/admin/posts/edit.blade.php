...
@section('content')
    <div class="row min-vh-80">
        <div class=" col-12 m-auto">
            <h3 class="mt-3 mb-0 text-center">Update Post</h3>
            <p class="lead font-weight-normal opacity-8 mb-7 text-center">This information will let us know more about you.</p>
            <div class="card">
                <div class="card-body">
                    <form class="multisteps-form__form" style="height: 391px;" method="post" action="{{route('admin.posts.update', $post->id)}}">
                        @csrf
                        @method('put')
                        <div class="row mt-3">
                            <div class="col-12 ">
                                <div class="input-group input-group-dynamic">
                                    ...
                                    <input value="{{$post->name}}" class="multisteps-form__input form-control" type="text" name="name" onfocus="focused(this)" onfocusout="defocused(this)">
                                </div>
                                @error('name')
                            ...
                            <div class="input-group input-group-static mt-4">
                                ...
                                <select class="form-control" name = "category_ids[]">
                                    ...
                                    @foreach($categories as $item)
                                        <option {{$post->categories->contains('id', $item->id) ? 'selected' : ''}} value="{{$item->id}}"> {{$item->name}}</option>
                                    @endforeach
                                </select>
                                @error('category_ids')
                                <div class="text-danger">{{$message}}</div>
